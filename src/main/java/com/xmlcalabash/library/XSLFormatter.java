...
public class XSLFormatter extends DefaultStep {
    ...
    public void run() throws SaxonApiException {
        ...
        if (provider == null) {
            throw new XProcException(step.getNode(), new RuntimeException("Failed to instantiate FO provider", pexcept));
        }
            ...
            URI id = store.writeEntry(href, base, contentType, new DataWriter() {
                public void store(OutputStream content) throws IOException {
                    ...
                    try {
                        ...
                        throw new XProcException(step.getNode(), new RuntimeException("Failed to process FO document", e));
                    } finally {
                        out.close();
                    }
                }
