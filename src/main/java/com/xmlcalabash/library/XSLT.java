...
public class XSLT extends DefaultStep {
    ...
    public void run() throws SaxonApiException {
        ...
        try {
            ...
            compiler.setErrorReporter(new ReportCompileErrors());
            XsltExecutable exec;
            try {
    ...
    private class ReportCompileErrors implements ErrorReporter {
        public void report(XmlProcessingError error) {
            if (error.isWarning())
                // log warning
                logger.warn(error.getMessage());
            else if (error.getFatalErrorMessage() != null)
                // report fatal error
                step.reportError(new XProcException(error, error.getFatalErrorMessage()));
            else
                // log error
    ...
    private class LogDynamicErrors implements ErrorReporter {
        public void report(XmlProcessingError error) {
            if (error.isWarning())
                logger.warn(error.getMessage());
            else
